...
def main(test=False):
    ...
    for dir, steps in moves:
        for step in range(steps):
            # Move the head knot
            knot_pos[0] = move_head(knot_pos[0], direction[dir])
            # Move all the trailing knots
            for k in range(1, num_knots):

                knot_pos[k] = move_tail(knot_pos[k - 1], knot_pos[k])

            tail_positions.add(knot_pos[-1])

            if test:
                print(knot_pos[0], knot_pos[k - 1])


    if test:
        print(tail_positions)

    print(f'Part 2 (number of tail positions: {len(tail_positions)}')
